...
export default function TransformerCard({ transformer }: { transformer: Transformer }) {
  ...
  const offlineDevicesCount = transformer.devices?.filter(d => d.status === 'offline').length || 0;
  const hasOfflineDevices = offlineDevicesCount > 0;
  
  return (
    <div className={`bg-[#16161a] p-6 rounded-2xl border ${hasAlerts || hasOfflineDevices ? 'border-rose-500/30' : 'border-[#2d2d33]'} hover:border-blue-500/50 transition-all shadow-xl group`}>
      <div className="flex justify-between items-start mb-6">
        <div className="p-3 bg-[#1e293b] rounded-xl text-blue-400 group-hover:scale-110 transition-transform">
          <Server size={24} />
        </div>
        <div className={`px-3 py-1 rounded-full text-[10px] font-black tracking-widest uppercase border ${hasAlerts || hasOfflineDevices ? 'bg-rose-500/10 text-rose-400 border-rose-500/20' : 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20'}`}>
          {hasAlerts ? 'Alert Active' : hasOfflineDevices ? 'Issue' : 'Healthy'}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-8">
        ...
        <div className="p-3 bg-[#111114] rounded-xl border border-[#2d2d33] relative overflow-hidden">
          ...
          {hasOfflineDevices && (
            <div className="absolute top-1 right-2 text-[8px] font-black text-rose-400 animate-pulse">
